import { dt as ExtensionType, ft as extensions$1, ht as __exportAll, j as DOMAdapter, ut as eventemitter3_default } from "./Geometry-9wnSDHOD.js";
import { bc as LoaderParserPriority, yc as path } from "./lib-Bp2l2r-f.js";
import { i as Ticker, s as CanvasSource } from "./Filter-B6V96S8x.js";
...
//#endregion
//#region node_modules/@pixi/sound/lib/filters/DistortionFilter.mjs
var DistortionFilter = class extends Filter {
	...
	set amount(value) {
		this._amount = value;
		if (getInstance().useLegacy) return;
...
//#endregion
//#region node_modules/@pixi/sound/lib/webaudio/WebAudioUtils.mjs
...
//#endregion
//#region node_modules/@pixi/sound/lib/filters/EqualizerFilter.mjs
var _EqualizerFilter = class extends Filter {
	...
	*/
	constructor(f32 = 0, f64 = 0, f125 = 0, f250 = 0, f500 = 0, f1k = 0, f2k = 0, f4k = 0, f8k = 0, f16k = 0) {
		...
		if (!getInstance().useLegacy) bands = equalizerBands.map((band) => {
		...
		for (let i = 0; i < this.bands.length; i++) {
			...
			if (i > 0) this.bands[i - 1].connect(node);
	...
	*/
	setGain(frequency, gain = 0) {
		if (!this.bandsMap[frequency]) throw new Error(`No band found for frequency ${frequency}`);
...
//#endregion
//#region node_modules/@pixi/sound/lib/filters/MonoFilter.mjs
...
//#endregion
//#region node_modules/@pixi/sound/lib/filters/ReverbFilter.mjs
...
//#endregion
//#region node_modules/@pixi/sound/lib/filters/StereoFilter.mjs
var StereoFilter = class extends Filter {
	...
	set pan(value) {
		this._pan = value;
		if (this._stereo) WebAudioUtils.setParamValue(this._stereo.pan, value);
		else if (this._panner) this._panner.setPosition(value, 0, 1 - Math.abs(value));
	}
	get pan() {
		return this._pan;
	}
...
//#endregion
//#region node_modules/@pixi/sound/lib/filters/StreamFilter.mjs
...
//#endregion
//#region node_modules/@pixi/sound/lib/filters/TelephoneFilter.mjs
...
//#endregion
//#region node_modules/@pixi/sound/lib/filters/index.mjs
...
//#endregion
//#region node_modules/@pixi/sound/lib/htmlaudio/HTMLAudioInstance.mjs
var id$1 = 0;
var _HTMLAudioInstance = class extends eventemitter3_default {
	...
	*/
	set(name, value) {
		if (this[name] === void 0) throw new Error(`Property with name ${name} does not exist.`);
		else switch (name) {
			case "speed":
				this.speed = value;
				break;
	...
	*/
	...
	stop() {
		this._internalStop();
		if (this._source) this.emit("stop");
	}
	...
	*/
	...
	play(options) {
		...
		if (end) console.assert(end > start, "End time is before start time");
		this._speed = speed;
		this._volume = volume;
		this._loop = !!loop;
		this._muted = muted;
	...
	*/
	_onUpdate() {
		...
		if (this._source.currentTime >= this._end && !this._source.loop) this._onComplete();
	}
	/**
...
var HTMLAudioMedia = class extends eventemitter3_default {
	init(parent) {
		...
		if (parent.url) this._source.src = parent.url;
	}
	create() {
	...
	*/
	...
	load(callback) {
		...
		if (!sound.url) {
			callback(/* @__PURE__ */ new Error("sound.url or sound.source must be set"));
			return;
		}
		...
		const onLoad = () => {
			...
			sound.isLoaded = true;
			const instance = sound.autoPlayStart();
			if (callback) callback(null, sound, instance);
		};
		const onAbort = () => {
			removeListeners();
			if (callback) callback(/* @__PURE__ */ new Error("Sound loading has been aborted"));
		};
		const onError = () => {
			removeListeners();
...
//#endregion
//#region node_modules/@pixi/sound/lib/htmlaudio/index.mjs
var htmlaudio_exports = /* @__PURE__ */ __exportAll({
	HTMLAudioContext: () => HTMLAudioContext,
	HTMLAudioInstance: () => HTMLAudioInstance,
	HTMLAudioMedia: () => HTMLAudioMedia
});
//#endregion
//#region node_modules/@pixi/sound/lib/SoundSprite.mjs
...
//#endregion
//#region node_modules/@pixi/sound/lib/utils/supported.mjs
var extensions = [
	"ogg",
	"oga",
	"opus",
...
var mimes = ["audio/mpeg", "audio/ogg"];
var supported = {};
...
var WebAudioInstance = class extends eventemitter3_default {
	...
	*/
	play(options) {
		...
		} else if (end) this._source.start(0, start, end - start);
		else this._source.start(0, start);
		this.emit("start");
		this._update(true);
	...
	enableTicker(enabled) {
		...
		if (enabled) Ticker.shared.add(this._updateListener, this);
	}
	/** The current playback progress from 0 to 1. */
	get progress() {
	...
	_update(force = false) {
		if (this._source) {
			...
			if (delta > 0 || force) {
				...
				} else progress = this._elapsed % duration / duration;
...
//#endregion
//#region node_modules/@pixi/sound/lib/Filterable.mjs
	...
	set filters(filters) {
		...
		if (filters && filters.length) {
			...
			filters.forEach((filter) => {
				if (prevFilter === null) this._input.connect(filter.destination);
				else prevFilter.connect(filter.destination);
				prevFilter = filter;
			});
...
//#endregion
//#region node_modules/@pixi/sound/lib/webaudio/WebAudioNodes.mjs
var _WebAudioNodes = class extends Filterable {
	...
	*/
	...
	destroy() {
		...
		if (this._script) this._script.disconnect();
...
var WebAudioMedia = class {
	...
	*/
	...
	load(callback) {
		if (this.source) this._decode(this.source, callback);
		else if (this.parent.url) this._loadUrl(callback);
		else if (callback) callback(/* @__PURE__ */ new Error("sound.url or sound.source must be set"));
		else console.error("sound.url or sound.source must be set");
	}
	...
	_decode(arrayBuffer, callback) {
		const audioBufferReadyFn = (err, buffer) => {
			if (err) {
				...
				if (callback) callback(null, this.parent, instance);
			}
		};
		if (arrayBuffer instanceof AudioBuffer) audioBufferReadyFn(null, arrayBuffer);
		else this.parent.context.decode(arrayBuffer, audioBufferReadyFn);
	}
};
//#endregion
//#region node_modules/@pixi/sound/lib/Sound.mjs
var _Sound = class {
	/**
	...
	*/
	static from(source) {
		let options = {};
		if (typeof source === "string") options.url = source;
		else if (source instanceof ArrayBuffer || source instanceof AudioBuffer || source instanceof HTMLAudioElement) options.source = source;
		else if (Array.isArray(source)) options.url = source;
		else options = source;
		options = {
			autoPlay: false,
		...
		return new _Sound(getInstance().useLegacy ? new HTMLAudioMedia() : new WebAudioMedia(), options);
	}
	/**
	* Use `Sound.from`
	* @ignore
	*/
	constructor(media, options) {
		...
		if (options.sprites) this.addSprites(options.sprites);
		if (this.preload) this._preload(options.loaded);
	}
	/**
	...
	*/
	preferUrl(urls) {
		...
		})).filter(({ ext }) => supported[ext]).sort((a, b) => extensions.indexOf(a.ext) - extensions.indexOf(b.ext));
		if (!file) throw new Error("No supported file type found");
		return file.url;
	}
	...
	*/
	addSprites(source, data) {
		if (typeof source === "object") {
			const results = {};
			for (const alias in source) results[alias] = this.addSprites(alias, source[alias]);
			return results;
		}
	...
	*/
	removeSprites(alias) {
		if (!alias) for (const name in this._sprites) this.removeSprites(name);
		else {
	...
	stop() {
		...
		this.isPlaying = false;
		for (let i = this._instances.length - 1; i >= 0; i--) this._instances[i].stop();
		return this;
	}
	play(source, complete) {
		let options;
		if (typeof source === "string") options = {
			sprite: source,
			loop: this.loop,
			complete
		};
		...
		if (options.offset) options.start = options.offset;
		if (!this.isLoaded) {
			if (this._preloadQueue) return new Promise((resolve) => {
			...
			return new Promise((resolve, reject) => {
				this._preload((err, sound, media) => {
					...
					if (err) reject(err);
						...
						if (options.loaded) options.loaded(err, sound, media);
						resolve(media);
					}
				});
			});
		}
		if (this.singleInstance || options.singleInstance) this._removeInstances();
	...
	refresh() {
		...
		for (let i = 0; i < len; i++) this._instances[i].refresh();
	}
	/** Handle changes in paused state. Internal only. */
	refreshPaused() {
		const len = this._instances.length;
		for (let i = 0; i < len; i++) this._instances[i].refreshPaused();
	}
	...
	autoPlayStart() {
		let instance;
		if (this.autoPlay) instance = this.play(this._autoPlayOptions);
		return instance;
	}
	/** Removes all instances. */
	_removeInstances() {
		for (let i = this._instances.length - 1; i >= 0; i--) this._poolInstance(this._instances[i]);
	...
	*/
	_poolInstance(instance) {
		instance.destroy();
		if (_Sound._pool.indexOf(instance) < 0) _Sound._pool.push(instance);
	}
};
var Sound = _Sound;
/** Pool of instances */
...
var WebAudioContext = class WebAudioContext extends Filterable {
	constructor() {
		const win = window;
	...
	*/
	playEmptySound() {
		...
		source.start(0, 0, 0);
		if (source.context.state === "suspended") source.context.resume();
	}
	/**
	...
	*/
	...
	destroy() {
		super.destroy();
		const ctx = this._ctx;
		if (typeof ctx.close !== "undefined") ctx.close();
	...
	*/
	set paused(paused) {
		if (paused && this._ctx.state === "running") this._ctx.suspend();
		else if (!paused && this._ctx.state === "suspended") this._ctx.resume();
		this._paused = paused;
	}
	get paused() {
		return this._paused;
	...
	*/
	decode(arrayBuffer, callback) {
		...
		if (result) result.catch(handleError);
	}
};
//#endregion
//#region node_modules/@pixi/sound/lib/SoundLibrary.mjs
var SoundLibrary = class {
	...
	*/
	init() {
		if (this.supported) this._webAudioContext = new WebAudioContext();
	...
	*/
	get filtersAll() {
		if (!this.useLegacy) return this._context.filters;
		return [];
	}
	set filtersAll(filtersAll) {
		if (!this.useLegacy) this._context.filters = filtersAll;
	}
	/**
	...
	*/
	_getOptions(source, overrides) {
		...
		else if (source instanceof ArrayBuffer || source instanceof AudioBuffer || source instanceof HTMLAudioElement) options = { source };
		else options = source;
		options = {
			...options,
			...overrides || {}
	...
	*/
	stopAll() {
		for (const alias in this._sounds) this._sounds[alias].stop();
		return this;
	}
	/**
	...
	*/
	exists(alias, assert = false) {
		...
		if (assert) console.assert(exists, `No sound matching alias '${alias}'.`);
		return exists;
	}
	/**
	...
	*/
	isPlaying() {
		for (const alias in this._sounds) if (this._sounds[alias].isPlaying) return true;
		return false;
	}
	/**
	...
	*/
	speed(alias, speed) {
		const sound = this.find(alias);
		if (speed !== void 0) sound.speed = speed;
		return sound.speed;
	}
	/**
...
//#endregion
//#region node_modules/@pixi/sound/lib/utils/playOnce.mjs
var PLAY_ID = 0;
...
//#endregion
//#region node_modules/@pixi/sound/lib/utils/render.mjs
function render(sound, options) {
	...
	const textureSource = new CanvasSource({ resource: canvas });
	if (!(sound.media instanceof WebAudioMedia)) return textureSource;
...
//#endregion
//#region node_modules/@pixi/sound/lib/utils/sineTone.mjs
function sineTone(hertz = 200, seconds = 1) {
	const sound = Sound.from({ singleInstance: true });
	if (!(sound.media instanceof WebAudioMedia)) return sound;
	const media = sound.media;
	...
	for (let i = 0; i < fArray.length; i++) {
		const angle = hertz * (i / buffer.sampleRate) * 2 * Math.PI;
...
//#endregion
//#region node_modules/@pixi/sound/lib/utils/index.mjs
...
//#endregion
//#region node_modules/@pixi/sound/lib/webaudio/index.mjs
var webaudio_exports = /* @__PURE__ */ __exportAll({
...
//#endregion
//#region node_modules/@pixi/sound/lib/soundAsset.mjs
var getAlias = (asset) => {
	const src = asset.src;
	let alias = asset?.alias?.[0];
	if (!alias || asset.src === alias) alias = path.basename(src, path.extname(src));
	return alias;
};
var soundAsset = {
	...
	loader: {
		...
		test(url) {
			return !!supported[path.extname(url).slice(1)] || mimes.some((mime) => url.startsWith(`data:${mime}`));
		},
...
//#endregion
export { Filter, Filterable, Sound, SoundLibrary, SoundSprite, filters_exports as filters, htmlaudio_exports as htmlaudio, sound, soundAsset, utils_exports as utils, webaudio_exports as webaudio };
